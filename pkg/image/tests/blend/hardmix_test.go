...
func TestBlendHardMix(t *testing.T) {
	testCases := []blendTestCase{
		...
		},
		{
			colors: Translucent,
			compositing: map[op.BlendCompositing]color.NRGBA{
				op.CompositeAll:         c(0xaa_40_bf_c0),
				op.CompositeBlendAndSrc: c(0xea_15_d5_80),
				op.CompositeBlendAndDst: c(0xbf_2b_ea_80),
				op.CompositeBlendOnly:   c(0xff_00_ff_40),
			},
			tolerance: 1,
		},
	}

	runBlendTest(t, "HardMix", testCases, nrgba.BlendHardMix, rgba.BlendHardMix)
}
